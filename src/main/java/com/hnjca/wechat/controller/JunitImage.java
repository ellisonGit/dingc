...
public class JunitImage {
        ...
        public static void testImage()  {
            markImageByText("早餐", "D:/beijing.png", "D:beijing2.png", 45, new Color(0, 0, 0), "PNG");
        }
    ...
    public static void markImageByText(String logoText, String srcImgPath,String newImagePath,Integer degree,Color color,String formaName) {
        ...
        try {
            ...
            // 4、设置水印旋转
            if (null != degree) {
                g.rotate(Math.toRadians(degree),  buffImg.getWidth()/12,buffImg.getHeight() /12);
            }
            // 5、设置水印文字颜色
            g.setColor(color);
            // 6、设置水印文字Font
            g.setFont(new Font("宋体", Font.BOLD, buffImg.getHeight() /12));
            // 7、设置水印文字透明度
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, 0.15f));
            // 8、第一参数->设置的内容，后面两个参数->文字在图片上的坐标位置(x,y)
            g.drawString(logoText,  buffImg.getWidth()/12 , buffImg.getHeight()/12);
            // 9、释放资源
            g.dispose();
            // 10、生成图片
